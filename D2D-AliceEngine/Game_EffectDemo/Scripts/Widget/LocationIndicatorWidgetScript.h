...
private:
	...
	// 진행 계산 (플레이어 기반은 비활성화, 데모 용도로 시간 기반 사용)
	bool  m_hasStartX = false; // 유지하지만 사용하지 않음
	float m_startX = 0.0f;
	...
	// 데모용 시간 기반 진행 값
	float m_demoTime = 0.0f;     // 누적 시간
	float m_demoSpeed = 0.25f;   // 초당 진행 비율(1.0f는 1초에 바 한 바퀴)
	bool  m_pingPong = true;     // 왕복 여부(true면 0->1->0, false면 0->1 반복)
};
