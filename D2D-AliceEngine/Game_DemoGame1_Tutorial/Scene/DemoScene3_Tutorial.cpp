...
void DemoScene3_Tutorial::Initialize()
{
	__super::Initialize();
}

void DemoScene3_Tutorial::Release()
{
	__super::Release();
}

void DemoScene3_Tutorial::Update()
{
	__super::Update();
	...
	if (m_player->GetComponent<Rigidbody2D>())
	{
		switch (rigidType)
		{
		case Define::ERigidBodyType::Static: rigidTypeStr = L"Static"; break;
		case Define::ERigidBodyType::Kinematic: rigidTypeStr = L"Kinematic"; break;
		case Define::ERigidBodyType::Dynamic: rigidTypeStr = L"Dynamic"; break;
		default: rigidTypeStr = L"Unknown"; break;
		}
	}
	...
	m_widget4->GetComponent<TextRenderComponent>()->SetText(
		L" [info] \n"
		L" player Position : (" + std::to_wstring(pos.x) + L", " + std::to_wstring(pos.y) + L")\n"
		L" player Rotation : (" + std::to_wstring(rotation) + L")\n"
		L" player Scale : (" + std::to_wstring(scale.x) + L", " + std::to_wstring(scale.y) + L")\n"
		L" player movement : (" + playerMovement + L")\n"
		L" player Speed : (" + playerSpeed + L")\n"
		L" player Gravity : ( -9.8 * " + playerGravity + L")\n"
		L" Rigidbody Type : " + rigidTypeStr
	);
	for (int i = 0; i < enemyMax; i++)
	{
		if (m_enemies[i]->GetComponent<Rigidbody2D>()->m_eRigidBodyState == Define::ERigidBodyState::Ground)
		{
			if(m_enemies[i]->GetComponent<Collider>()) m_enemies[i]->GetComponent<Collider>()->SetBoxColor(FColor::Green);
		}
		else if (m_enemies[i]->GetComponent<Rigidbody2D>()->m_eRigidBodyState == Define::ERigidBodyState::OnRigidBody)
		{
			if(m_enemies[i]->GetComponent<Collider>()) m_enemies[i]->GetComponent<Collider>()->SetBoxColor(FColor::Blue);
		}
		else
		{
			if(m_enemies[i]->GetComponent<Collider>()) m_enemies[i]->GetComponent<Collider>()->SetBoxColor(FColor::Red);
		}
	}
	if (m_player->GetComponent<Rigidbody2D>())
	{
		...
		{
			if(m_player->GetComponent<Collider>()) m_player->GetComponent<Collider>()->SetBoxColor(FColor::Green);
		}
		else
		{
			if(m_player->GetComponent<Collider>()) m_player->GetComponent<Collider>()->SetBoxColor(FColor::Red);
		}
	}
}

void DemoScene3_Tutorial::OnEnter()
{
	__super::OnEnter();
	...
	for (int i = 0; i < enemyMax; i++)
	{
		...
		m_enemies[i]->transform()->SetPosition(FRandom::GetRandomPointInCircle2D(300, 300, 200.0f));
...
void DemoScene3_Tutorial::OnExit()
{
	__super::OnExit();
	m_enemies.clear();
}

void DemoScene3_Tutorial::EnemyInput()
{
}

void DemoScene3_Tutorial::PlayerInput()
{
