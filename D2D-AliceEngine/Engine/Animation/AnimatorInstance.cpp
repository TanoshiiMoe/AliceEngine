...
void AnimatorInstance::Initialize()
{
	__super::Initialize();
}

void AnimatorInstance::Update(const float& deltaSeconds)
{
	...
	for (const auto& trans : currentStateDef.transitions) {
		...
		if (EvaluateConditions(trans.conditions)) {
			...
			for (auto& condition : trans.conditions) {
				if (condition.type == Define::ParameterType::Trigger) {
...
void AnimatorInstance::Release()
{
	__super::Release();
}

void AnimatorInstance::Render()
{
	__super::Render();
}
...
{
	m_TriggerParameters[name] = false;
}
...
bool AnimatorInstance::CheckCondition(const Condition& condition)
{
	...
	{
	case Define::ParameterType::Bool:
	{
	...
	case Define::ParameterType::Float:
	{
	...
	case Define::ParameterType::Trigger:
	{
